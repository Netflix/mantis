...
| `Scale up below percentage`   | When the aggregated value for all workers rises above this value, the stage will scale up. |
...
PID control system uses a continuous feedback loop to maintain a signal at a target level. Mantis offers variations of
this strategy that operations on different signals. Additionally, they try to learn the appropriate target over time
...
It still respects cooldown between scaling actions.

### Clutch

The strategy operates on CPU, Memory, Network and UserDefined. Every 24 hours, it will pick 1 dominant resource and use the
P99 value as the target set point. For the next 24 hours, it will monitors that resource metric and scales the stage to
keep the metric at the target set point. For the initial 24 hours after the job is first launched, this strategy will
scale the stage to max in order to learn the first dominant resource and set point.
...
no initial 24 hour pin high period, the PID controller can start working right away. You can supply the configuration as
...
drops and lag at 0. It takes the first 10 minutes after job launch to learn the first RPS set point. Afterwards, it may
adjust the set point once every hour. Set point should become stable the longer a job runs, since it simply takes
a percentile of historical RPS metric.
...
Optionally, tt is possible to further customize the behavior of the PID controller. You can supply the configuration as
JSON in the job parameter `mantis.jobmaster.clutch.config`. Example:

```json
{
  "rpsConfig": {
    "setPointPercentile": 0.50,
    "scaleDownBelowPct": 0.4,
    "scaleUpAbovePct": 0.1,
...
| Field                | Description  |
| -------------------- | ------------ |
| `setPointPercentile` | Percentile of historical RPS metric to use as the set point. Default is `0.75`. |
| `scaleDownBelowPct`  | Only scale down if the PID controller output is below this number. It can be used to delay a scaling action. Valid input is between `[0.0, 1.0]`. Default is `0.0`. |
| `scaleUpAbovePct`    | Only scale up if the PID controller output is above this number. It can be used to delay a scaling action. Valid input is between `[0.0, 1.0]`. Default is `0.0`. |
