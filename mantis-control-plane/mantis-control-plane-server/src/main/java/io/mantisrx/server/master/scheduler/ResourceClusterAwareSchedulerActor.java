...
class ResourceClusterAwareSchedulerActor extends AbstractActorWithTimers {
  ...
  public static Props props(
    ...
    return Props.create(ResourceClusterAwareSchedulerActor.class, resourceCluster, executeStageRequestFactory,
        jobMessageRouter);
  }
  ...
  private void onFailedScheduleRequestEvent(FailedToScheduleRequestEvent event) {
    log.error("Failed to submit the request {}; Retrying", event.getScheduleRequest(),
        event.getThrowable());
    getTimers()
        .startSingleTimer("Retry-Schedule-Request-For" + event.getScheduleRequest().getWorkerId().toString(), event.onRetry(), Duration.ofMinutes(1));
  }
